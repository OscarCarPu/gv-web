...
describe('habits schemas', () => {
	it('HabitWithLogListSchema parses list', () => {
		const result = HabitWithLogListSchema.parse([
			{ id: 1, name: 'Read', description: null, log_value: 1, frequency: 'daily', target_min: null, target_max: null, recording_required: true, period_value: 1, current_streak: 3, longest_streak: 10 }
		]);
	...
	it('CreateHabitResponseSchema parses response', () => {
		const result = CreateHabitResponseSchema.parse({ id: 1, name: 'Read', description: null, frequency: 'daily', target_min: null, target_max: null, recording_required: true });
