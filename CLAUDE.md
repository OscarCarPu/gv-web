...
- **Stopping a print is two requests**: PrusaLink addresses a job by id, so `DELETE /domotics/printers/[id]/job` asks `GET /api/v1/job` first (204 = idle → 409 "No print is running") and only then `DELETE /api/v1/job/{id}`. Never take a job id from the client — a page left open since the last job would cancel whatever started since. The button itself arms on the first click and stops on the second (`.btn-stop-print.is-armed`): the app has no confirmation dialogs, but a stray click must not throw away a six-hour print
