...
- **Task dependencies**: Tasks can depend on other tasks. `depends_on` (tasks this depends on) and `task_depends` (tasks depending on this) are `TaskDepRef[]` (`{id, name, due_at}`) on all task responses. API accepts `depends_on: number[]` on create/update. Zod schemas use `.nullable().transform(v => v ?? [])` since API may return null. Editing reverse deps (task_depends) requires fetching+updating each affected task individually.
